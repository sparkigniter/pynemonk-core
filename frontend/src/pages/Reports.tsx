        ...
        <div className="space-y-6 animate-fade-in-up">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Generate comprehensive reports for attendance, finance, and academics.</p>
                </div>
            ...
            <div className="card p-8 flex flex-col items-center justify-center min-h-[500px] text-center bg-gradient-to-br from-slate-50 to-white">
                <div className="relative mb-6">
                    ...
                    <div className="w-20 h-20 bg-white rounded-2xl shadow-xl flex items-center justify-center relative z-10 text-theme-primary rotate-3 hover:rotate-0 transition-transform">
                        <BarChart2 size={40} />
                    </div>
                </div>
                <h3 className="text-xl font-bold text-slate-800 font-heading mb-2">Advanced Reporting Studio</h3>
                <p className="text-slate-500 max-w-md mb-8">Build custom reports, visualize data trends, and export insights. The analytics engine is currently being provisioned for your tenant.</p>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-2xl">
                    <div className="p-4 bg-white border border-slate-100 rounded-xl shadow-sm text-left">
                        <PieChart className="text-amber-500 mb-3" size={24} />
                        <h4 className="font-semibold text-slate-800 mb-1">Financial Health</h4>
                        <p className="text-xs text-slate-500">Revenue, expenses, and fee collection trends.</p>
                    </div>
                    <div className="p-4 bg-white border border-slate-100 rounded-xl shadow-sm text-left">
                        <Calendar className="text-emerald-500 mb-3" size={24} />
                        <h4 className="font-semibold text-slate-800 mb-1">Attendance Stats</h4>
                        <p className="text-xs text-slate-500">Daily attendance rates and chroninc absenteeism.</p>
                    </div>
                    <div className="p-4 bg-white border border-slate-100 rounded-xl shadow-sm text-left">
                        <TrendingUp className="text-blue-500 mb-3" size={24} />
                        <h4 className="font-semibold text-slate-800 mb-1">Academic Perform</h4>
                        <p className="text-xs text-slate-500">Exam scores, subject averages, and term results.</p>
                    </div>
                </div>
            </div>
        </div>
