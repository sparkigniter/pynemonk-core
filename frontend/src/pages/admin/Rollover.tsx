...
export default function Rollover() {
    ...
    return (
        <div className="max-w-6xl mx-auto animate-fade-in pb-12">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
                <div>
                    <h1 className="text-4xl font-black text-slate-800 tracking-tight flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-600 flex items-center justify-center shadow-lg shadow-theme/20">
                            <RefreshCw className="w-6 h-6 text-white animate-spin-slow" />
                        </div>
                        Academic Session Rollover
                    </h1>
                    <p className="text-[var(--text-muted)] mt-2 text-lg font-medium">Transition school data and students to the next academic cycle</p>
                </div>

                {/* Stepper */}
                <div className="flex items-center gap-1 bg-[var(--card-bg)]/60 backdrop-blur-md p-1.5 rounded-2xl border border-[var(--card-border)]/60 shadow-sm">
                    {[
                        ...
                        return (
                            <div key={s.id} className="flex items-center">
                                <div className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-300 ${isActive ? 'bg-primary text-white shadow-md shadow-theme/20' : isPast ? 'text-primary' : 'text-[var(--text-muted)]'}`}>
                                    <div className={`w-6 h-6 rounded-lg flex items-center justify-center ${isActive ? 'bg-[var(--card-bg)]/20' : isPast ? 'bg-primary/5' : 'bg-slate-100'}`}>
            ...
            {/* Main Content Card */}
            <div className="card overflow-hidden border-none shadow-2xl shadow-theme/10/50 bg-[var(--card-bg)]/80 backdrop-blur-xl relative">
                {/* Decorative gradients */}
                <div className="absolute top-0 right-0 w-96 h-96 bg-primary/50/5 blur-[120px] -mr-48 -mt-48 pointer-events-none" />
                ...
                <div className="p-8 md:p-12">
                        ...
                        <div className="space-y-12 animate-fade-in-up">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                <div className="space-y-6">
                                    ...
                                    <p className="text-[var(--text-muted)] text-sm">Select the current academic session you want to roll over from.</p>
                                    ...
                                    <div className="space-y-3">
                                        {years.map(y => (
                                            <button
                                                ...
                                                className={`w-full p-5 rounded-2xl border-2 transition-all text-left flex items-center justify-between group ${sourceYear === y.id ? 'border-primary bg-primary/5/50 shadow-lg shadow-theme/10' : 'border-[var(--card-border)] bg-slate-50/50 hover:border-[var(--card-border)]'}`}
                                            >
                                                <div>
                                                    <div className="flex items-center gap-3">
                                                        <span className={`text-lg font-bold ${sourceYear === y.id ? 'text-indigo-900' : 'text-slate-700'}`}>{y.name}</span>
                                                        {y.is_current && (
                                                            <span className="px-2 py-0.5 rounded-full bg-primary text-[10px] text-white font-black uppercase tracking-wider shadow-sm">Active Now</span>
                                                        )}
                                                    </div>
                                                    <p className="text-xs text-[var(--text-muted)] mt-1">{new Date(y.start_date).getFullYear()} — {new Date(y.end_date).getFullYear()}</p>
                                                </div>
                                                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${sourceYear === y.id ? 'border-primary bg-primary' : 'border-slate-300'}`}>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <p className="text-[var(--text-muted)] text-sm">Select or create the next academic session for your school.</p>
                                        ...
                                        <div className="space-y-3">
                                            {years.filter(y => y.id !== sourceYear).map(y => (
                                                <button
                                                    ...
                                                    className={`w-full p-5 rounded-2xl border-2 transition-all text-left flex items-center justify-between group ${targetYear === y.id ? 'border-violet-600 bg-violet-50/50 shadow-lg shadow-violet-100' : 'border-[var(--card-border)] bg-slate-50/50 hover:border-[var(--card-border)]'}`}
                                                >
                                                    <div>
                                                        <span className={`text-lg font-bold ${targetYear === y.id ? 'text-violet-900' : 'text-slate-700'}`}>{y.name}</span>
                                                        <p className="text-xs text-[var(--text-muted)] mt-1">Available for rollover</p>
                                                    </div>
                                            ...
                                            <button 
                                                ...
                                                className="w-full p-6 rounded-2xl border-2 border-dashed border-[var(--card-border)] hover:border-indigo-400 hover:bg-primary/5/30 transition-all text-left group flex items-center gap-4"
                                            >
                                                <div className="w-12 h-12 rounded-xl bg-slate-50 flex items-center justify-center group-hover:bg-primary/10 transition-colors">
                                                    <Calendar className="w-6 h-6 text-[var(--text-muted)] group-hover:text-primary" />
                                                </div>
                                                <div>
                                                    <p className="font-bold text-slate-800">Create New Academic Year</p>
                                                    <p className="text-xs text-[var(--text-muted)] mt-0.5">Quickly set up the next session {newYearForm.name}</p>
                                                </div>
                                            </button>
                                        </div>
                                    ) : (
                                        <div className="p-6 bg-slate-50/80 rounded-[32px] border border-[var(--card-border)]/60 space-y-5 animate-scale-in shadow-inner">
                                            <div className="space-y-1.5">
                                                <label className="text-xs font-bold text-[var(--text-muted)] px-1 uppercase tracking-wider">Session Name</label>
                                                <input 
                                                    type="text" 
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-1.5">
                                                    <label className="text-xs font-bold text-[var(--text-muted)] px-1 uppercase tracking-wider">Start Date</label>
                                                    <input 
                                                        type="date" 
                                                ...
                                                <div className="space-y-1.5">
                                                    <label className="text-xs font-bold text-[var(--text-muted)] px-1 uppercase tracking-wider">End Date</label>
                                                    <input 
                                                        type="date" 
                            ...
                            <div className="pt-8 border-t border-[var(--card-border)] flex justify-end">
                                <button
                                    onClick={handleFetchPreview}
                                    disabled={!sourceYear || !targetYear || loading}
                                    className="bg-primary hover:bg-primary disabled:opacity-50 text-white font-black py-4 px-12 rounded-2xl shadow-xl shadow-theme/20 transition-all flex items-center gap-3 active:scale-95 h-16 text-lg"
                                >
                    ...
                    {step === 'configure' && (
                        <div className="space-y-10 animate-fade-in-up">
                            <div className="flex flex-col md:flex-row items-center gap-8 p-8 bg-primary/5/50 border border-indigo-100 rounded-[40px]">
                                <div className="w-20 h-20 rounded-3xl bg-primary flex items-center justify-center shadow-2xl shadow-theme/20 shrink-0">
                                    <Zap className="w-10 h-10 text-white" />
                                </div>
                                <div>
                                    <h3 className="text-2xl font-black text-slate-800">Smart Data Migration</h3>
                                    <p className="text-[var(--text-muted)] mt-1">
                                        You are rolling over from <span className="text-primary font-bold">{years.find(y => y.id === sourceYear)?.name}</span> to <span className="text-violet-600 font-bold">{years.find(y => y.id === targetYear)?.name}</span>.
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {[
                                    ...
                                    <button
                                        ...
                                        className={`p-8 rounded-[32px] border-2 transition-all text-left flex flex-col gap-5 h-full ${options[opt.id as keyof typeof options] ? 'border-primary bg-[var(--card-bg)] shadow-xl shadow-theme/10/50' : 'border-[var(--card-border)] bg-slate-50/30 opacity-60 hover:opacity-100'}`}
                                    >
                                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${options[opt.id as keyof typeof options] ? 'bg-primary text-white shadow-lg shadow-theme/20' : 'bg-slate-100 text-[var(--text-muted)]'}`}>
                                        ...
                                        <div className="pt-4 flex items-center justify-between border-t border-[var(--card-border)]">
                                            <div className="flex flex-col">
                                                <span className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Impact</span>
                                                <span className="text-sm font-bold text-primary">{opt.count} Items</span>
                                            </div>
                                            <div className={`w-12 h-6 rounded-full p-1 transition-all flex items-center ${options[opt.id as keyof typeof options] ? 'bg-primary' : 'bg-slate-200'}`}>
                                                <div className={`w-4 h-4 bg-[var(--card-bg)] rounded-full shadow-sm transition-all transform ${options[opt.id as keyof typeof options] ? 'translate-x-6' : 'translate-x-0'}`} />
                                            </div>
                                        </div>
                                    </button>
                                ))}
                            ...
                            <div className="bg-slate-50/50 rounded-[32px] border border-[var(--card-border)]/60 p-8">
                                <div className="flex items-center justify-between mb-6">
                                    <h4 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                                        <Info className="w-5 h-5 text-primary" />
                                        Data Preview
                                    </h4>
                                    <span className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-widest">Live Breakdown</span>
                                </div>
                                
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    <div className="space-y-4">
                                        <p className="text-xs font-black text-[var(--text-muted)] uppercase tracking-widest flex items-center gap-2">
                                            <LayoutIcon className="w-3.5 h-3.5 text-primary" /> Classrooms to be Created
                                        </p>
                                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                            {preview?.classrooms?.length > 0 ? preview.classrooms.map((c: any, i: number) => (
                                                <div key={i} className="px-4 py-3 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl flex items-center justify-between text-sm shadow-sm">
                                                    <span className="font-bold text-slate-700">{c.name}</span>
                                                    <span className="text-[var(--text-muted)] text-xs">{c.grade_name} · Cap: {c.capacity}</span>
                                                </div>
                                            )) : (
                                                <div className="py-8 text-center text-[var(--text-muted)] text-sm font-medium italic bg-[var(--card-bg)]/50 rounded-2xl border border-dashed border-[var(--card-border)]">No classrooms found to clone</div>
                                            )}
                                        </div>
                                    </div>
                                    <div className="space-y-4">
                                        ...
                                        <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                            {preview?.assignments?.length > 0 ? preview.assignments.map((a: any, i: number) => (
                                                <div key={i} className="px-4 py-3 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl flex items-center justify-between text-sm shadow-sm">
                                                    ...
                                                    <span className="text-[var(--text-muted)] text-xs">{a.classroom_name}</span>
                                                </div>
                                            )) : (
                                                <div className="py-8 text-center text-[var(--text-muted)] text-sm font-medium italic bg-[var(--card-bg)]/50 rounded-2xl border border-dashed border-[var(--card-border)]">No staff mappings found</div>
                                            )}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="pt-10 border-t border-[var(--card-border)] flex justify-between items-center">
                                <button 
                                    onClick={() => setStep('select-years')} 
                                    className="flex items-center gap-2 text-[var(--text-muted)] hover:text-slate-800 font-bold transition-all group"
                                >
                                ...
                                <button
                                    ...
                                    className="bg-primary hover:bg-primary text-white font-black py-4 px-12 rounded-2xl shadow-xl shadow-theme/20 transition-all flex items-center gap-3 active:scale-95 h-16 text-lg"
                                >
                                    Proceed to Review
                                    <ArrowRight className="w-5 h-5" />
                                </button>
                    ...
                    {step === 'review' && (
                        <div className="space-y-10 animate-scale-in">
                            <div className="text-center space-y-3 mb-10 max-w-2xl mx-auto">
                                ...
                                <p className="text-[var(--text-muted)] font-medium">Please review the summary below. This action is significant as it will switch the active academic session for the entire school platform.</p>
                            </div>

                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                <div className="space-y-6">
                                    <div className="bg-primary p-8 rounded-[40px] text-white shadow-2xl shadow-theme/20 relative overflow-hidden">
                                ...
                                <div className="space-y-6 flex flex-col">
                                    ...
                                    <div className="pt-4 space-y-4">
                                        <button
                                            ...
                                            disabled={loading}
                                            className="w-full bg-surface-dark hover:brightness-110 text-white font-black py-5 rounded-2xl shadow-2xl shadow-slate-200 transition-all flex items-center justify-center gap-3 text-lg h-20 active:scale-[0.98]"
                                        >
                                        ...
                                        <button 
                                            ...
                                            className="w-full text-[var(--text-muted)] hover:text-slate-800 font-bold transition-all py-2 text-sm"
                                        >
                    ...
                    {step === 'success' && (
                        <div className="text-center py-16 space-y-10 animate-scale-in">
                            ...
                            <div className="space-y-4 max-w-xl mx-auto">
                                <h2 className="text-5xl font-black text-[var(--text-main)] tracking-tight">Session Rolled Out!</h2>
                                <p className="text-xl text-[var(--text-muted)] font-medium">
                                    Academic Year <span className="text-primary font-black">{targetYearData?.name}</span> is now fully operational and set as the school's primary session.
                                </p>
                            </div>
                            ...
                            <div className="pt-8 flex flex-col md:flex-row items-center justify-center gap-4">
                                <button
                                    ...
                                    className="bg-primary text-white font-black py-5 px-16 rounded-2xl hover:bg-primary transition-all shadow-2xl shadow-theme/20 h-20 active:scale-95"
                                >
                                    Go to Dashboard
                                </button>
                                <button
