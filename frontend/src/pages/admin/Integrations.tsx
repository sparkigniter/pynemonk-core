                    ...
                    <div className="card p-4">
                        <div className="relative mb-4">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-50 border border-[var(--card-border)] text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
                            />
                        </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0
                                            ${isActive ? 'bg-primary text-white' : 'bg-slate-100 text-[var(--text-muted)]'}`}>
                                            <Zap size={18} />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <p className={`text-sm font-bold truncate ${isActive ? 'text-primary' : 'text-[var(--text-main)]'}`}>{item.name}</p>
                        ...
                        <>
                            ...
                            <div className="card p-6 flex flex-col md:flex-row md:items-center gap-6">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2">
                                        <h2 className="text-xl font-bold text-[var(--text-main)] font-heading">{currentIntegration?.name}</h2>
                                        <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${currentIntegration?.isEnabled ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-[var(--text-muted)]'}`}>
                                            {currentIntegration?.isEnabled ? 'Active' : 'Disabled'}
                                        </span>
                                    </div>
                                    <p className="text-sm text-[var(--text-muted)] mt-1">{currentIntegration?.description}</p>
                                </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                <div className="card p-5 border-l-4 border-l-primary">
                                    <p className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Total Students</p>
                                    <div className="flex items-end gap-2 mt-2">
                                        <p className="text-2xl font-black text-[var(--text-main)] leading-none">{health?.total || 0}</p>
                                        <span className="text-[10px] text-[var(--text-muted)] mb-1">In current year</span>
                                    </div>
                                </div>
                                <div className="card p-5 border-l-4 border-l-emerald-500">
                                    <p className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Mapped Identities</p>
                                    <div className="flex items-end gap-2 mt-2">
                                        <p className="text-2xl font-black text-[var(--text-main)] leading-none">{health?.mapped || 0}</p>
                                ...
                                <div className="card p-5 border-l-4 border-l-amber-500">
                                    <p className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider">Missing SATS IDs</p>
                                    <div className="flex items-end gap-2 mt-2">
                                        <p className="text-2xl font-black text-[var(--text-main)] leading-none">{health?.unmapped || 0}</p>
                            ...
                            <div className="card overflow-hidden">
                                <div className="px-6 py-4 border-b border-[var(--card-border)] flex items-center justify-between bg-slate-50/50">
                                        ...
                                        health.validationErrors.map((error, idx) => (
                                            <div key={idx} className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-sm font-bold text-[var(--text-main)]">{error.entityName}</p>
                                ...
                                <div className="px-6 py-3 bg-slate-50 border-t border-[var(--card-border)] flex justify-center">
                                    <button className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest hover:text-slate-600 transition-colors">
                                        View All Issues
                                    </button>
                                </div>
                            </div>
