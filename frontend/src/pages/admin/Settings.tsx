                        ...
                        <div className="flex-1 p-6 space-y-10 overflow-y-auto">
                            ...
                            <div className="space-y-6">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h3 className="text-sm font-bold text-slate-900">Attendance Tracking Mode</h3>
                                        <p className="text-xs text-slate-500 mt-1">Determine how attendance is recorded across your institution.</p>
                                    </div>
                                    <div className="flex bg-slate-100 p-1.5 rounded-2xl gap-1">
                                        <button
                                            onClick={() => setSchoolSettings((p: any) => ({ ...p, attendance_mode: 'DAILY' }))}
                                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${schoolSettings.attendance_mode === 'DAILY' ? 'bg-white text-primary shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                        >
                                            Daily
                                        </button>
                                        <button
                                            onClick={() => setSchoolSettings((p: any) => ({ ...p, attendance_mode: 'PERIOD_WISE' }))}
                                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${schoolSettings.attendance_mode === 'PERIOD_WISE' ? 'bg-white text-primary shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                                        >
                                            Period-wise
                                        </button>
                                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className={`p-6 rounded-[2rem] border-2 transition-all ${schoolSettings.attendance_mode === 'DAILY' ? 'border-primary/20 bg-primary/5' : 'border-slate-50 opacity-60'}`}>
                                        <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-sm mb-4"><ListChecks className="text-primary w-5 h-5" /></div>
                                        <h4 className="text-sm font-bold text-slate-900 mb-1">Daily Attendance (K-12)</h4>
                                        <p className="text-[10px] text-slate-500 leading-relaxed">Attendance is taken once a day by the Class Teacher. Ideal for primary and secondary schools.</p>
                                    </div>
                                    <div className={`p-6 rounded-[2rem] border-2 transition-all ${schoolSettings.attendance_mode === 'PERIOD_WISE' ? 'border-primary/20 bg-primary/5' : 'border-slate-50 opacity-60'}`}>
                                        <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-sm mb-4"><LayoutGrid className="text-primary w-5 h-5" /></div>
                                        <h4 className="text-sm font-bold text-slate-900 mb-1">Period-wise (Colleges)</h4>
                                        <p className="text-[10px] text-slate-500 leading-relaxed">Attendance is taken for every subject slot in the timetable. Ideal for colleges and universities.</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 border-t border-slate-100 pt-8">
                                <div className="space-y-4">
                                    ...
                                    <div className="p-5 bg-slate-50 rounded-2xl border border-dashed border-slate-200 flex items-center justify-between">
                                        <div>
                                            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Sample Format</p>
