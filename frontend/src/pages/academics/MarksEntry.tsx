...
export default function MarksEntry() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                setExam(examData);
                const currentPaper = examData.papers.find(p => p.id === parseInt(paperId!));
    ...
    const toggleAbsent = (studentId: number) => {
        setMarks(prev => ({
            ...prev,
            [studentId]: { ...prev[studentId], isAbsent: !prev[studentId].isAbsent, score: !prev[studentId].isAbsent ? '' : prev[studentId].score }
        }));
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await examApi.saveMarks(parseInt(id!), parseInt(paperId!), marksData);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h1 className="text-4xl font-black text-slate-900 tracking-tight leading-none">Evaluation: {paper?.subject_name}</h1>
                    </div>
                </div>
                        ...
                        {students.map((student) => {
                            ...
                            return (
                                <tr key={student.student_id} className={`group transition-colors ${isExcluded ? 'opacity-50' : 'hover:bg-slate-50/50'}`}>
                                    ...
                                    <td className="px-8 py-5">
                                        <span className="text-xs font-black text-slate-600 uppercase tracking-widest">{student.classroom_name} {student.classroom_section}</span>
                                    </td>
                                    <td className="px-8 py-5">
                                        <button 
                                            disabled={isExcluded}
                                    ...
                                    <td className="px-8 py-5">
                                        {isExcluded ? (
                                            ...
                                            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest
                                                ${isPassing ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                                                {isPassing ? 'Pass' : 'Fail'}
                                            </span>
                                        ) : (
