    ...
    if (isLoading) {
        return (
            <div className="h-[80vh] flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-[var(--text-muted)] font-black uppercase tracking-widest text-[10px]">Assembling Candidate Matrix...</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-8 bg-slate-50 min-h-[calc(100vh-100px)] rounded-[3rem] border border-[var(--card-border)] shadow-2xl relative flex flex-col">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <h1 className="text-3xl font-black text-[var(--text-main)] tracking-tight">Exam Invitations</h1>
                        ...
                        <p className="text-[var(--text-muted)] font-medium mt-1">Manage individual candidate eligibility and invitation status.</p>
                    </div>
                </div>

                <div className="flex items-center gap-3">
                    <button className="flex items-center gap-2 px-6 py-3.5 bg-[var(--card-bg)] border border-[var(--card-border)] text-slate-600 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-slate-50 transition-all shadow-sm">
                        <Download size={16} />
                        Export List
                    </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                <div className="bg-[var(--card-bg)] p-6 rounded-[2rem] border border-[var(--card-border)] shadow-sm flex items-center gap-5">
                    <div className="bg-theme-primary/5 text-theme-primary p-4 rounded-2xl">
                        <Users size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-1">Total Candidates</p>
                        <p className="text-2xl font-black text-[var(--text-main)]">{pagination.total}</p>
                    </div>
                </div>
                <div className="bg-[var(--card-bg)] p-6 rounded-[2rem] border border-[var(--card-border)] shadow-sm flex items-center gap-5 opacity-50">
                    <div className="bg-emerald-50 text-emerald-500 p-4 rounded-2xl">
                        <UserCheck size={24} />
                    </div>
                    <div>
                        <p className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest mb-1">Active Status</p>
            ...
            <div className="flex flex-col gap-6 mb-8 px-2">
                <div className="flex items-center justify-between gap-6">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input
                            ...
                            className="w-full pl-12 pr-6 py-4 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl text-sm font-bold shadow-sm focus:outline-none focus:ring-4 focus:ring-theme-primary/10 transition-all"
                            value={search}
                            onChange={(e) => setSearch(e.target.value)}
                        />
                    </div>
                    <div className="flex p-1 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl shadow-sm">
                        {(['all', 'included', 'excluded'] as const).map((f) => (
                            <button
                                key={f}
                                onClick={() => setStatusFilter(f)}
                                className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${statusFilter === f ? 'bg-surface-dark text-white shadow-lg shadow-theme/10' : 'text-[var(--text-muted)] hover:text-slate-600'}`}
                            >
                                {f}
                            </button>
                        ))}
                ...
                {/* Granular Filters */}
                <div className="flex items-center gap-4 bg-[var(--card-bg)] p-4 rounded-[2rem] border border-[var(--card-border)] shadow-sm overflow-x-auto">
                    <div className="flex items-center gap-2 px-4 text-[var(--text-muted)] shrink-0">
                        <Filter size={16} />
                        <span className="text-[10px] font-black uppercase tracking-widest">Filters</span>
                    </div>

                    <select
                        className="bg-slate-50 border border-[var(--card-border)] px-4 py-2 rounded-xl text-xs font-bold outline-none focus:ring-2 focus:ring-theme-primary/20 shrink-0"
                        value={gradeFilter || ''}
                        onChange={(e) => {
                    ...
                    <div className="flex items-center gap-2 pr-4 shrink-0">
                        <span className="text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Page {pagination.page} of {pagination.pages}</span>
                        <div className="flex items-center bg-slate-50 border border-[var(--card-border)] rounded-xl overflow-hidden shadow-sm">
                            <button
                                onClick={() => setPagination(p => ({ ...p, page: Math.max(1, p.page - 1) }))}
                                disabled={pagination.page === 1}
                                className="p-2 hover:bg-[var(--card-bg)] text-[var(--text-muted)] hover:text-slate-600 disabled:opacity-20 transition-all border-r border-[var(--card-border)]"
                            >
                                <ChevronLeft size={16} />
                            </button>
                            <button
                                ...
                                className="p-2 hover:bg-[var(--card-bg)] text-[var(--text-muted)] hover:text-slate-600 disabled:opacity-20 transition-all"
                            >
            ...
            {/* Table */}
            <div className="flex-1 bg-[var(--card-bg)] rounded-[3rem] border border-[var(--card-border)] shadow-xl shadow-slate-200/20 overflow-hidden flex flex-col mb-4 relative">
                {isDataLoading && (
                    <div className="absolute inset-0 bg-[var(--card-bg)]/40 backdrop-blur-[1px] z-20 flex items-center justify-center">
                            ...
                            <tr className="border-b border-[var(--card-border)]">
                                <th className="px-8 py-6 w-16">
                                    <button
                                        ...
                                        className={`w-5 h-5 rounded border-2 transition-all flex items-center justify-center ${selectedStudents.size === students.length && students.length > 0 ? 'bg-theme-primary border-theme-primary' : 'bg-[var(--card-bg)] border-[var(--card-border)]'}`}
                                    >
                                        {selectedStudents.size === students.length && students.length > 0 && <Check size={12} className="text-white" strokeWidth={4} />}
                                    </button>
                                </th>
                                <th className="px-8 py-6 text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Candidate</th>
                                <th className="px-8 py-6 text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Grade \ Section</th>
                                <th className="px-8 py-6 text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Admission No</th>
                                <th className="px-8 py-6 text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest">Invitation Status</th>
                                <th className="px-8 py-6 text-[10px] font-black text-[var(--text-muted)] uppercase tracking-widest text-right">Action</th>
                            </tr>
                        </thead>
                            ...
                            {students.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="py-24 text-center">
                                        ...
                                        <h4 className="text-lg font-black text-[var(--text-main)] tracking-tight">No candidates found</h4>
                                        <p className="text-[var(--text-muted)] text-sm font-medium mt-1">Adjust your filters to find specific students.</p>
                                    </td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={student.student_id} className={`group hover:bg-slate-50/50 transition-all ${selectedStudents.has(student.student_id) ? 'bg-theme-primary/5' : ''}`}>
                                        <td className="px-8 py-6">
                                            <button
                                                ...
                                                className={`w-5 h-5 rounded border-2 transition-all flex items-center justify-center ${selectedStudents.has(student.student_id) ? 'bg-theme-primary border-theme-primary' : 'bg-[var(--card-bg)] border-[var(--card-border)] group-hover:border-slate-300'}`}
                                            >
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-tighter">Student Profile</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase">{student.classroom_section} Section</span>
                                            </div>
                                        </td>
