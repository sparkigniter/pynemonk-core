    ...
    return (
        <div
            ...
            className={`
                ...
                ${isDragging ? 'opacity-40 scale-95 border-dashed border-primary/30' : `${color} border-transparent shadow-sm hover:shadow-md hover:-translate-y-0.5`}
            `}
        >
            ...
            <div className="flex-1 min-w-0">
                ...
                {subtitle && <p className="text-[10px] font-black opacity-40 uppercase tracking-tighter truncate mt-0.5">{subtitle}</p>}
            </div>
        </div>
    );
};
...
const DraggableEntry = ({ entry, onDelete }: { entry: TimetableEntry; onDelete: (id: number) => void }) => {
    ...
    return (
        <div
            ...
            {...listeners}
            className={`
                h-full w-full rounded-xl p-2.5 shadow-lg flex flex-col justify-between overflow-hidden group/entry cursor-grab active:cursor-grabbing border
                ${isDraft ? 'bg-white border-primary border-dashed text-primary shadow-primary/5' : 'bg-gradient-to-br from-primary to-accent text-white border-white/20 shadow-primary/20'}
            `}
        >
            <div>
                <p className={`text-[11px] font-black truncate uppercase tracking-tight leading-tight ${isDraft ? 'text-primary' : 'text-white'}`}>{entry.subject_name}</p>
                <div className="flex items-center gap-1 mt-1 opacity-80">
                    <User className={`w-2.5 h-2.5 ${isDraft ? 'text-primary/60' : 'text-white/60'}`} />
                    <p className={`text-[9px] font-bold truncate ${isDraft ? 'text-primary/80' : 'text-white/80'}`}>{entry.teacher_name}</p>
                </div>
            </div>
            <div className="flex items-center justify-between mt-auto pt-1">
                <span className={`text-[9px] font-black uppercase tracking-tighter ${isDraft ? 'text-primary/40' : 'text-white/40'}`}>
                    {entry.start_time.slice(0, 5)}
                </span>
                <button
                    onPointerDown={(e) => { e.stopPropagation(); entry.id && onDelete(entry.id); }}
                    className={`p-1 transition-all rounded-lg ${isDraft ? 'text-primary/20 hover:text-primary hover:bg-primary/5' : 'text-white/40 hover:text-white hover:bg-white/10'}`}
                >
...
const DroppableSlot = ({ id, children }: { id: string; children?: React.ReactNode }) => {
    ...
    return (
        <div
            ref={setNodeRef}
            className={`
                p-1.5 border-r border-slate-100 relative group/cell min-h-[110px] transition-all duration-300
                ${isOver ? 'bg-primary/5 ring-4 ring-primary/20 ring-inset z-10' : 'hover:bg-slate-50/50'}
            `}
        >
            ...
            <div className="absolute top-1 right-1 opacity-0 group-hover/cell:opacity-100 transition-opacity">
                <div className="bg-slate-200/50 p-1 rounded-md">
